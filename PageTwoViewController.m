...
@interface PageTwoViewController (ViewHandlingMethods)
- (void)prevController;
- (void)nextController;
...
//	self.scrollView = nil;
	...
	[super viewDidUnload];
	NSLog(@"Count of scrollView : %lu",(unsigned long)[scrollView retainCount]);
}
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    self.title = NSLocalizedString(@"Site 2", @"");
...
- (void)viewDidLoad
{
	...
	[scrollView setContentSize:CGSizeMake(320, 3160)];
...
- (void) prevController {
	...
	PageOneViewController *pageoneVC = [[PageOneViewController alloc] initWithNibName: @"PageOneViewController" bundle:nil];
	...
	pageoneVC.hidesBottomBarWhenPushed = YES;
	[navigationStack addObject:pageoneVC];
...
-(void)loadAudioFile {
	...
	// Load the audio file
	if(player == nil){	
		NSString *audioFile = [NSString stringWithFormat:@"nolac2"];
